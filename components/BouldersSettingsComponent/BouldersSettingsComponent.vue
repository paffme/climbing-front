...
export default class BouldersSettingsComponent extends Vue {
  ...
  onBoulderingChange(val: APIBoulderingRounds, oldVal: APIBoulderingRounds) {
    if (!val) return
    this.categoriesCanBeSelected = this.extractCategoryGenre(val)
    let diff: APIBoulderingRounds | undefined
    if (oldVal) {
      diff = this.difference(val, oldVal)
      const extracted = this.extractCategoryGenre(diff)
      ...
      this.updateDisplayedCategories(this.currentCategoryDisplay)
      return
    }
    this.updateDisplayedCategories({
      category: this.categoriesCanBeSelected[0].category,
  ...
  async onOpenModalEditRound(round: BoulderingRoundInputEdit) {
    console.log('onOpenModalEditRound', round)
    this.$buefy.modal.open({
      parent: this,
      props: {
        round
  ...
  private roundEditDto(round: BoulderingRoundInputEdit): any {
    return {

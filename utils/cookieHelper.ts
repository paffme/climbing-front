...
export function createCookie(key: string, value: string, expires = 3600, path = '/'): void {
  console.log('createCookie = ', `${key}=${value}; path=${path}; expires=${formatExpire(expires)}`)
  document.cookie = `${key}=${value}; path=${path}; expires=${formatExpire(expires)}`
}
...
export function createCookieFromObject(
  ...
  document.cookie = `${key}=${JSON.stringify(value)}; path=${path}; expires=${formatExpire(expires)}`
}
